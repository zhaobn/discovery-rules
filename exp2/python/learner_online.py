...
def run_condition_online(model='base', condition='simple', num_actions=40, seed=0):
    print(f"Running {model} model for condition: {condition}, with seed {seed}")
    np.random.seed(seed)

    # Initialize stats
    cum_rewards = np.zeros(num_actions)
    highst_levels = np.zeros(num_actions)
    epi_probs = np.zeros(num_actions)
    recover_rate = np.zeros(num_actions)
    ...
    for t in range(num_actions):
        ...
        # update belief per each action
        if t > 0:        
            if model == 'pcfg':
                if len(prior_mdps) == 0:
                    print("No valid prior MDPs left.")
                    cum_rewards[t] = cum_rewards[t-1]
                    highst_levels[t] = highst_levels[t-1]
                    break
                else:
                    counts = prior_mdps['count']
                    norm_probs = counts / counts.sum()
        ...
        epi_probs[t] = sampled_transitions.mean()
        recover_rate[t] = (true_transitions == sampled_transitions).mean()
    ...
    return condition, log_cum_rewards, highst_levels, epi_probs, recover_rate


# %%
run_condition_online('pcfg', 'simple', 40, 0)
    ...
    for seed in range(n_seeds):
        ...
        for action_idx in range(actions):
            results.append({
                ...
                'seed': seed,
                'action': action_idx,
                'log_cum_rewards': log_cum_rewards[action_idx],
                'highest_levels': highst_levels[action_idx],
